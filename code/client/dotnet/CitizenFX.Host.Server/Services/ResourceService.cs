...
internal class ResourceService
{
	...
	public void ShutdownResource(string resourceName)
	{
		...
		if (resource.CitizenResource != null)
		{
			resource.CitizenResource.OnStop();
		}
	...
	public void LoadResourceFile(string resourceName, string fileName)
	{
		...
		if (resource == null)
		{
			throw new Exception($"Tried to load resource file {fileName} for resource {resourceName} which is not registered.");
		}
		...
		if (resource.AssemblyLoadContext != null)
		{
			throw new Exception($"Resource {resourceName} is already loaded.");
		}
		...
		var wrapperAssembly = assemblyLoadContext.LoadFromAssemblyPath(Path.Combine(currentPath, "CitizenFX.Wrapper.dll"));
