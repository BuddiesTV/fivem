...
public static unsafe class Library
{
	...
	[UnmanagedFunctionPointer(CallingConvention.Cdecl)]
	public delegate void RuntimeEventHandlerDelegate(IntPtr eventName, IntPtr eventPayload, uint payloadSize,
		IntPtr eventSource);
	...
	private static void Initialize(IntPtr readyAssemblyWrapperFnPtr, IntPtr printFnPtr, IntPtr setEventHandlerFnPtr,
		IntPtr setTickHandlerFnPtr, IntPtr getNativeHandlerFnPtr, IntPtr invokeNativeFnPtr,
		IntPtr invokeFunctionReferenceFnPtr)
	{
		_readyAssemblyWrapperFn =
			Marshal.GetDelegateForFunctionPointer<ReadAssemblyDelegate>(readyAssemblyWrapperFnPtr);
		...
		_invokeFunctionReferenceFn =
			Marshal.GetDelegateForFunctionPointer<InvokeFunctionReferenceDelegate>(invokeFunctionReferenceFnPtr);
	...
	{
		if (!IsInitialized) throw new InvalidOperationException("Library is not initialized.");

		return NativeHandlers.GetOrAdd(nativeHash, hash =>
		{
			var handler = _getNativeHandlerFn(hash);
			if (handler == IntPtr.Zero) throw new Exception($"Failed to get native handler for hash {hash}.");
	...
	public static byte[] ReadAssemblyRaw(IntPtr runtimePointer, string assemblyName)
	{
		...
		var success = _readyAssemblyWrapperFn(runtimePointer, assemblyName, out var assemblyDataPtr,
			out var dataSize);

		if (!success) throw new Exception($"Failed to read assembly {assemblyName}.");
	...
	public static void InvokeNative(ulong nativeHash, params object[] args)
	{
		...
		if (nativeHandler == IntPtr.Zero) throw new Exception($"Failed to get native handler for hash {nativeHash}.");
		...
		var ptr = scriptContext.initialArguments;
		...
		try
		{
			for (var i = 0; i < args.Length; i++)
			{
				...
				{
					var strPtr = Marshal.StringToHGlobalAnsi(strValue);
		...
		{
			foreach (var pointer in allocatedPointers) Marshal.FreeHGlobal(pointer);
		}
	}
