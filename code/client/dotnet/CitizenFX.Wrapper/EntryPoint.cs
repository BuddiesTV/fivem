...
public static class EntryPoint
{
	...
	public static void Main(IResource resource, byte[] assemblyData)
	{
		...
		if (resource.AssemblyLoadContext == null) throw new Exception($"Resource {resource.Name} is not loaded.");

		using var ms = new MemoryStream(assemblyData);
		var assembly = resource.AssemblyLoadContext.LoadFromStream(ms);

		var resourceMain = assembly.GetTypes()
			.FirstOrDefault(x => x.IsClass && typeof(CitizenResource).IsAssignableFrom(x));
		...
		resource.CitizenResource = resourceInstance ??
		                           throw new Exception(
			                           $"Failed to create instance of resource main class {resourceMain.FullName}.");
